...
test("TC_REG_01: Register with valid data", async ({ page }) => {
  ...
  const email = `nongwin${Date.now()}@example.com`;
...
test("TC_REG_02: Register with existing email", async ({ page }) => {
  const register = new RegisterPage(page);
  await register.navigate();
...
test("TC_REG_05: Register with invalid first name (contains nunmbers)", async ({
  page,
}) => {
  const register = new RegisterPage(page);
  await register.navigate();
  ...
  await register.expectErrorPopup("REG_1003", `Invalid Formats: ${firstName}`);
});
